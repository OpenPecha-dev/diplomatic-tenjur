...
def update_diplomat_tenjur(text_id):
    google_pecha_idx = yaml.safe_load((G_PEDURMA_OPF_PATH / f'P000792.opf/index.yml').read_text(encoding='utf-8'))
    g_meta = yaml.safe_load((G_PEDURMA_OPF_PATH / f'P000792.opf/meta.yml').read_text(encoding='utf-8'))
    derge_hfmls = get_hfml_text(DERGE_OPF_PATH, text_id)
    google_hfmls = get_hfml_text(G_PEDURMA_OPF_PATH, text_id, google_pecha_idx)
